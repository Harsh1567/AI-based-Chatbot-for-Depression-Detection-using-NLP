...
headers = {"Authorization": "Bearer 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 "}
def query(payload):
    data = json.dumps(payload)
    response = requests.request("POST", API_URL, headers=headers, data=data)
    return json.loads(response.content.decode("utf-8"))
data = query("I love a girl but she loves someone else what should I do")
...
for x,y in(recursive_items(data)):
    if(x=='generated_text'):
        print(x,y)
        break
...
def chatPage(request):
    ...
    if request.method == 'POST':
        if('Send' in request.POST):
            ...
            chatres=query(userQuery)
            chat_API=[]
            cans=''
            uque=''
            for x,y in(recursive_items(chatres)):
                
                if(x=='generated_text'):
                    cans=y
                if(x=="past_user_inputs"):
                    uque=y[0]
            chat_API.extend([cans,uque])

            CHATS.append(chat_API)
